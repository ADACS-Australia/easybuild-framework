...
class HierarchicalLowercaseMNS(ModuleNamingScheme):
    ...
    def det_full_version(self, ec):
        ...
        return (versionprefix + ec['version'] + ec['versionsuffix']).lower()
    ...
    def det_toolchain_compilers_name_version(self, tc_comps):
        ...
        if tc_comps is None:
            ...
            res = (tc_comps[0]['name'].lower(), self.det_full_version(tc_comps[0]).lower())
        else:
        ...
        if is_compiler or non_dummy_cuda:
		    ...
		    if ec['name'] in comp_names:
	                ...
	                comp_name = comp_name.lower()
                        comp_ver_tmpl = comp_ver_tmpl.lower()
                        comp_versions = {ec['name'].lower(): self.det_full_version(ec)}
                        if ec['name'] in ['ifort', 'intel']:
                        ...
                        if non_dummy_tc:
                            ...
                            if tc_comp_name in [name.lower() for name in comp_names]:
                                # also provide toolchain version for non-dummy toolchains
                                comp_versions.update({tc_comp_name.lower(): tc_comp_ver})

                        comp_ver_keys = re.findall(r'%\((\w+)\)s', comp_ver_tmpl)
                        if all(comp_ver_key.lower() in comp_versions for comp_ver_key in comp_ver_keys):
            ...
            if tc_comp_info is None:
                ...
                paths.append(os.path.join(MPI, tc_comp_name, tc_comp_ver, ec['name'], fullver).lower())
    ...
    def is_short_modname_for(self, short_modname, name):
        ...
        """
        name = name.lower()
        short_modname = short_modname.lower()
